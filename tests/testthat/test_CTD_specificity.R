...
test_that("Correct specificity values of CDT calculated", {
  ...
  exp_set <- matrix(ifelse(rand_data<0.55,0,rand_data),ncol=10,
  ...
  grouping <- lapply(grouping,function(x) 
    as.character(sample(paste0("celltype_",1:4),10,replace=T)))
  
  #Now apply EWCE function
  fNames = EWCE::generate.celltype.data(exp=exp_set,
  ...
  known_ans <- list(setNames(c(0.4450643, 0.2458045, 0.3091312),
                             c("celltype_2","celltype_3","celltype_4")),
                    setNames(c(0.4050525, 0.4271945, 0.1677530),
                             c("celltype_1","celltype_2","celltype_4")),
                    setNames(c(0.2022591, 0.3428756, 0.1924686, 0.2623967),
                             c("celltype_1","celltype_2","celltype_3","celltype_4")),
                    setNames(c(0.1583313, 0.2530405, 0.3367327, 0.2518955),
                             c("celltype_1","celltype_2","celltype_3","celltype_4")))
