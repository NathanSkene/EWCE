...
test_that("bootstrap enrichment function error handling and geneSizeControl runs as expected", {
  ...
  set.seed(12345678)
  results = bootstrap.enrichment.test(sct_data=ctd,hits=human.hits,
  ...
  fail_return2 <- 
    tryCatch( bootstrap.enrichment.test(sct_data=ctd,hits=mouse.hits.fake,
                                        bg=mouse.bg,reps=reps,annotLevel=level,
                                        ),
              error=function(e) e, 
              warning=function(w) w)
  ...
  fail_return4 <- 
    tryCatch( bootstrap.enrichment.test(sct_data=ctd,hits=mouse.hits[1:3],
                                        bg=mouse.bg,reps=reps,annotLevel=level),
              error=function(e) e, 
              warning=function(w) w)
