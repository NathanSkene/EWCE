...
test_that("EWCE expression data creation", {
  ...
  up_celltypes <-c(names(tt_results$hit.cells.up)[tt_results$hit.cells.up==max(tt_results$hit.cells.up)],
                   names(tt_results$hit.cells.up)[tt_results$hit.cells.up==min(tt_results$hit.cells.up)])
  dwn_celltypes <-c(names(tt_results$hit.cells.down)[tt_results$hit.cells.down==max(tt_results$hit.cells.down)],
