...
# Note
If you have previously used the EWCE package from [neurogenomics/EWCE](https://github.com/neurogenomics/EWCE). This package, which is available on Bioconductor (3.13, R >= 4.1) offers functional improvements. One thing to note is that all functions which previously used dot '.' notations to separate words have been updated to underscores '_' (e.g, `bootstrap.enrichment.test()` is now `bootstrap_enrichment_test()`).  The package [vignette website](https://nathanskene.github.io/EWCE/articles/EWCE.html) gives further details.
