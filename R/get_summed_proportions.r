...
#'
#' \code{get_summed_proportions} Given the target geneset, randomly sample genelists of equal length, obtain the specificity of these and then
...
#' @param geneSizeControl Boolean. Should the sampled gene lists control for GC content and transcript length?
#' @param controlledCT Name of a celltype (from colnames of sct_data[[x]]$specificity.
#' @param control_network If geneSizeControl=TRUE, then must provide the control network
...
#' @export
get_summed_proportions <- function(hitGenes,sct_data,annotLevel,reps,geneSizeControl,controlledCT=NULL,control_network=NULL){
    bootstrap_data = matrix(0,ncol=length(colnames(sct_data[[annotLevel]]$specificity)),nrow=reps)
    ...
    # cell.list.dist gets the summed proportion of 'hitGenes' across all cell types at annotLevel

    # Check control_network provided if geneSizeControl=TRUE
    if(geneSizeControl==TRUE){
        if(is.null(control_network)){
            stop("ERROR: if geneSizeControl==TRUE then get_summed_proportions must be passed control_network as an argument")
        }
    }

    # If controlling for expression of another cell type, then the samples of bootstrapped genes must match the deciles of expression specificity
    if(!is.null(controlledCT)){
        controlled_bootstrap_set = generate_controlled_bootstrap_geneset(hitGenes,sct_data,annotLevel,reps,controlledCT)
    }
    ...
    for(s in 1:reps){
        ...
        bootstrap_data[s,] = cell.list.dist(bootstrap_set,sct_data,annotLevel)
    }
    colnames(bootstrap_data) = names(hit.cells)

    return(list(hit.cells=hit.cells,bootstrap_data=bootstrap_data,controlledCT=controlledCT))
}
