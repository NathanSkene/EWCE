...
#'
#' \code{add.res.to.merging.list} Adds EWCE results to a list for merging analysis
#'
#' @param full_res results list generated using \code{\link{bootstrap.enrichment.test}} or
#' \code{\link{ewce_expression_data}} functions. Multiple results tables can be merged into one
#' results table, as long as the 'list' column is set to distinguish them.
#' @param existing_results Output of previous rounds from adding results to list. Leave empty if
...
add.res.to.merging.list <- function(full_res,existing_results=NULL){
    ...
    if(is.null(full_res$bootstrap_data)){
        # If it is directional, and other results are in list, check they are all directional
        if(!is.null(existing_results)){
            for(i in 1:length(existing_results)){
                if(is.null(existing_results[[i]]$Direction)){stop("ERROR: Cannot merge directional with non-directional results table")}
            }
        }
        # Setup the new entries
        new_entry_up   = list(Direction="Up",bootstrap_data=full_res$bootstrap_data.up, hitCells=full_res$hit.cells.up)
        new_entry_down = list(Direction="Down",bootstrap_data=full_res$bootstrap_data.down, hitCells=full_res$hit.cells.down)
        ...
        # If it is NOT directional, and other results are in list, check they are all not directional
        if(!is.null(existing_results)){
            for(i in 1:length(existing_results)){
                if(!is.null(existing_results[[i]]$Direction)){stop("ERROR: Cannot merge directional with non-directional results table")}
            }
        ...
        # Setup the new entries
        new_entry   = list(bootstrap_data=full_res$bootstrap_data, hitCells=full_res$hit.cells)
