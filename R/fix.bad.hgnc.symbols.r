...
#' - Given an expression matrix, wherein the rows are supposed to be HGNC symbols, find those symbols which are not official HGNC symbols, then
#' correct them if possible. Return the expression matrix with corrected symbols.
#' @param exp An expression matrix where the rows are MGI symbols
#' @param dropNonHGNC Boolean. Should symbols not recognised as HGNC symbols be dropped?
...
fix.bad.hgnc.symbols <- function(exp,dropNonHGNC=FALSE){
    ...
    if(length(date_like)>0){
        print(sprintf("Possible corruption of gene names by excel: %s",paste(date_like,collapse=", ")))
        warning(sprintf("Possible corruption of gene names by excel: %s",paste(date_like,collapse=", ")))
    }
