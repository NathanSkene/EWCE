...
#'
#' \code{load.linnarsson.sct.data} Function which was specifically written for extracting the expression and annotation data from the publically downloadable
#' Linnarsson lab Cortex/Hippocampus dataset
#'
#' @param fName Path to the downloaded file
#' @return A list containing 'exp' and 'annot'.
#' \itemize{
#'   \item \code{exp}: Numerical matrix with row for each gene and column for each cell. Row names are MGI/HGNC gene symbols. Column names are cell IDs which can be cross referenced against the annot data frame.
#'   \item \code{annot}: Annotation data frame which must contain at least three columns: cell_id, level1class and level2class. Can also contain other columns.
#' }
#' @examples
#' \dontrun{
...
load.linnarsson.sct.data <- function(fName){
    ...
    if(paste(all_dat[1:9,2],collapse=",")!="group #,total mRNA mol,well,sex,age,diameter,cell_id,level1class,level2class"){stop("ERROR: annotation rows are not written as expected")}
    # A blank row seperates the annotation from expression data... find that row
    expDataStarts = 1
    for(i in 1:30){if(length(unique(unlist(all_dat[i,])))==1 & unique(unlist(all_dat[i,]))[1]==""){expDataStarts=i+1}}
    ...
    zeisel_annot = all_dat[1:(expDataStarts-2),2:dim(all_dat)[2]]
    ...
    z2_annot = data.frame(groupNo=as.numeric(zeisel_annot[1,]),
