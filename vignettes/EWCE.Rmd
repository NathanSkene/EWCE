...
Each level of a CTD corresponds to increasingly refined cell-type/-subtype annotations. For example, in the CTD  `ewceData::ctd()` level 1 includes the cell-type "interneurons", while level 2 breaks this group into 16 different interneuron subtypes ("Int...").
...
Plot the expression of three marker genes across all cell types in the CTD.
...
Gene lists input into *EWCE*  can come from any source (e.g. GWAS, candidate genes, pathways). 
...
We now run the cell type enrichment tests on the gene list. Since the CTD is from mouse data (and is annotated using mouse genes) we specify the argument `sctSpecies="mouse"`. `bootstrap_enrichment_test` will automatically convert the mouse genes to human genes.
