...
The user then obtains a gene set and a suitable background gene set. As the choice of gene sets is up to the user we do not provide functions for doing this. Appropriate choice of background set is discussed in the associatedpublication.

Bootstrapping is then performed using the `bootstrap.enrichment.test` function.

# Installing EWCE

The *EWCE* package is available on Bioconductor. To install the package one needs first to install R then run the following lines of code:
...
```{r }
#install data package for vignette
#library(devtools)
#install_github("neurogenomics/ewceData")
library(ewceData)
library(EWCE)
library(ggplot2)
library(cowplot)
